...
import { EArmModes, effortlesshomeArea, EAlarmEvent, Dictionary, effortlesshomeConfig, HomeAssistant } from '../types';
...
export const computeAreaDisplay = (
  area: string | number,
  areaConfig: Dictionary<effortlesshomeArea>,
  effortlesshomeConfig: effortlesshomeConfig
) => {
  if (area == 0) {
    return {
      name: effortlesshomeConfig.master.name,
      value: 0,
    };
...
export const getAreaOptions = (areaConfig: Dictionary<effortlesshomeArea>, effortlesshomeConfig: effortlesshomeConfig) => {
  let areas: (string | number)[] = [];

  const area_ids = Object.keys(areaConfig).filter(e => Object.values(areaConfig[e].modes).some(v => v.enabled));
  if (effortlesshomeConfig.master.enabled && area_ids.length > 1) areas = [...areas, 0];

  areas = [...areas, ...area_ids];
  return areas;
};

export const getArmModeOptions = (area: string | number | undefined, areaConfig: Dictionary<effortlesshomeArea>) => {
  const areaList = (areaCfg: effortlesshomeArea) =>
...
export const getWildcardOptions = (event?: EAlarmEvent, effortlesshomeConfig?: effortlesshomeConfig) => {
  ...
  if (
    !event ||
    (effortlesshomeConfig?.code_arm_required &&
      [EAlarmEvent.Armed, EAlarmEvent.Arming, EAlarmEvent.ArmFailure].includes(event)) ||
    (effortlesshomeConfig?.code_disarm_required && [EAlarmEvent.Disarmed, EAlarmEvent.Untriggered].includes(event))
  )
    options = [
      ...options,
      {
