...
async def createalarm(open_sensors: dict = None):
    ...
    match sensor_device_class:
        case "door":
            response = await createsecurityalarm(sensor_device_class, sensor_device_name)
        case "window":
            response = await createsecurityalarm(sensor_device_class, sensor_device_name)
        case "motion":
            response = await createsecurityalarm(sensor_device_class, sensor_device_name)
        case "moisture":
            response = await createmonitoringalarm(sensor_device_class, sensor_device_name)
        case "heat":
            response = await createmonitoringalarm(sensor_device_class, sensor_device_name)
        case "smoke":
            response =  await createmonitoringalarm(sensor_device_class, sensor_device_name)
...
async def createsecurityalarm(sensor_device_class: str, sensor_device_name: str):
    ...
    payload = '{"sensor_device_class":"'+ sensor_device_class +'", "sensor_device_name":"'+ sensor_device_name +'"}'
...
async def createmonitoringalarm(sensor_device_class: str, sensor_device_name: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=json.loads(payload)) as response:
            _LOGGER.debug("API response status: %s", response.status)
            _LOGGER.debug("API response headers: %s", response.headers)
            content = await response.text()
            _LOGGER.debug("API response content: %s", content)
...
class effortlesshomeAreaEntity(effortlesshomeBaseEntity):
    ...
    def async_trigger(self, skip_delay: bool = False, open_sensors: dict = None):
        ...
        if not entry_delay:
            ...
            result = asyncio.create_task(createalarm(open_sensors))
